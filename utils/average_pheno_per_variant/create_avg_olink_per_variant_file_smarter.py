...
# Step A:
olink_var_ids = pl.scan_parquet(f"/home/dnanexus/data_dir/genebass1e6_genes_10kb_allvars_annotated_250925.parquet").filter(pl.col('region').is_in(unique_phenotypes)).select(['id']).unique().collect(engine='streaming')['id'].to_list()
...
for chunk_num in tqdm(range(indices_vars2keep.shape[0]//chunk_size + 1)):
    indices_vars2keep_chunk = indices_vars2keep[chunk_num*chunk_size:(chunk_num+1)*chunk_size]
    process_genotype_chunk(
        geno=geno.oindex[indices_vars2keep_chunk, olink_indices],
        var_ids=var_ids[indices_vars2keep_chunk],
