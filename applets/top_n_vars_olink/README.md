...
Benchmarks variant scoring models by comparing cumulative protein z-scores when ranking variants by model scores (top-N analysis) on Olink proteomics.

## Overview

This applet ranks variants by annotation scores within each gene and computes cumulative protein expression effects (PROTRIDER-corrected). It evaluates whether higher-scoring variants (according to each annotation) tend to have stronger proteomic effects, comparing built-in scores against optional custom models.

Key features:
- Per-gene top-N rankings: variants ranked by score, proteins averaged over top N
...
|-------|----------|------|-------------|
| `associations_parquet` | Yes | file | Gene-protein associations with LOFTEE correlation. Columns: `region`, `phenotype`, `correlation`. |
| `annotations_parquet` | Yes | file | Variant annotations with all scores. Columns: `id`, `region`, + score columns. |
| `appv_parquet` | Yes | file | Average protein per variant (PROTRIDER-corrected). Columns: `id`, `phenotype`, `mean_pheno_value`, `n_individuals`. |
...
```bash
dx build applets/top_n_vars_olink/ --destination <project>:/applets/ -f
```

## Details

- Proteins are PROTRIDER-corrected and already normalized (no INT transformation)
