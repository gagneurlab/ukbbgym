...
def compute_correlations(
    ...
    pheno_df = pheno_corrected_df.unpivot(
        index=["sample_id"], variable_name="phenotype", value_name="value"
    )
    ...
    for gene_id in tqdm(assocs_df["gene_id"].unique(), desc="Correlations for genes"):
        ...
        phenos_needed = (
            assocs_df.filter(pl.col("gene_id") == gene_id)["phenotype"]
            .unique()
            .to_list()
        )
        pheno_filtered = pheno_df.filter(
            pl.col("phenotype").is_in(phenos_needed)
        ).lazy()
        cdf = bdf_filtered.join(pheno_filtered, on="sample_id", how="left")

        corr_df_list.append(
            compute_correlations_lazy(cdf)
            .with_columns(pl.lit(gene_id).alias("gene_id"))
            .collect()
        )
    ...
    annotation_category_map = {
        ann: category
        for category, anns in rare_variant_annotations_dict.items()
        if category != "misc"
        for ann in anns
    }
    ...
    corr_long = corr_df.unpivot(
        ...
        [
            pl.col("correlation_type")
            .str.extract(r"(pearson|spearman)")
            .alias("method"),
            pl.col("correlation_type")
            .str.extract(r"(sum|max|top2)")
            .alias("aggregation"),
        ]
    )
