...
Computes Spearman correlations between variant annotation scores and protein means for benchmarking variant scoring models on Olink proteomics.

## Overview

This applet analyzes how well annotation scores (built-in tools like AlphaMissense, REVEL, SpliceAI, etc., plus optional custom models) correlate with average protein expression (PROTRIDER-corrected, no INT) for variants in a set of gene-protein associations.

Key features:
- Supports Olink proteomics phenotypes (PROTRIDER-corrected, no INT transformation)
...
```bash
dx build applets/correlations_olink/ --destination <project>:/applets/ -f
```
...
- Filtering: `n_variants > 100` per gene-protein-annotation triple
